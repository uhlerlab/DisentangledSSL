...
def wrapper_train_mp(args, a, lam, seed, train_loader, test_loader, train_dataset, test_dataset):
    ...
    args.a = a
    set_seed(seed)
    logs = train_mp_joint(args, train_loader, test_loader, train_dataset, test_dataset)
    log_entry = {f'a_{a}_lambda_{lam}_seed_{seed}': logs}
    lock.acquire()
    try:
        log_file_path = f"./results_synthetic_task_baseline/{args.data_mode}/joint_training_logs.json"
...
def train_mp_joint(args, train_loader, test_loader, train_dataset, test_dataset):
    jointdisen = JointDisenModel(args.dim_info['X'], args.dim_info['Y'], args.hidden_dim, args.embed_dim, args.layers, args.activation, 
                        initialization = 'normal', distribution='vmf', vmfkappa=args.kappa, lr=args.lr,
                        lmd_start_value=args.lmd_start, lmd_end_value=args.lmd_end, lmd_n_iterations=8000, lmd_start_iteration=0,
                        a=args.a, ortho_norm=args.ortho_norm, condzs=args.condzs, proj=args.proj, usezsx=args.usezsx, apdzs=args.apdzs).cuda()
    joint_optim = optim.Adam(jointdisen.parameters(), lr=args.lr)
    logs = jointdisen.train_model(train_loader, train_dataset, test_dataset, joint_optim, num_epoch=args.num_epoch, noise_scale=args.noise_scale, drop_scale=args.drop_scale)
    return logs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--start_a', type=float, default=1)
    ...
    print(f'Testing for a={a_s} and lambdas={lambdas}')
    print(f'Total process running are {len(a_s)* len(lambdas) * len(seeds)} spread across GPUs {args.gpus}')
    parameters = [(args, a, lam, seed, train_loader, test_loader, train_dataset, test_dataset) for a, lam, seed in product(a_s, lambdas, seeds)]
