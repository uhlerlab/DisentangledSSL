...
def wrapper_train_mp(args, a, lam, seed, train_loader, test_loader, train_dataset, test_dataset):
    ...
    args.a = a
    set_seed(seed)
    logs = train_mp_focal(args, train_loader, test_loader, train_dataset, test_dataset)
    log_entry = {f'a_{a}_lambda_{lam}_seed_{seed}': logs}
    lock.acquire()
    try:
        log_file_path = f"./results_synthetic_task_baseline/{args.data_mode}/focal_training_logs.json"
...
def train_mp_focal(args, train_loader, test_loader, train_dataset, test_dataset):
    focal = Focal(args.dim_info['X'], args.dim_info['Y'], args.hidden_dim, args.embed_dim, args.layers, args.activation, 
                        initialization = 'normal', a=args.a, lmd=args.lmd).cuda()
    joint_optim = optim.Adam(focal.parameters(), lr=args.lr)
    logs = focal.train_model_focal(train_loader, train_dataset, test_dataset, joint_optim, num_epoch=args.num_epoch, noise_scale=args.noise_scale, drop_scale=args.drop_scale)
    return logs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--start_a', type=float, default=0)
